`timescale 1ns/1ps
// Self-checking TB for the 5-modulus Ansh-N core. Drives residue tuples derived
// from random integer operands, checks the reconstructed product against the
// TRUE (x*y) mod 41580, measures pipeline latency and throughput.
module tb;
    localparam integer M = 41580;
    localparam integer NTEST = 300000;   // random pairs (1.7B exhaustive is too many)
    ...
    // golden reference operands + expected (TB may divide; only the DUT must not)
    reg [31:0] cx = 0, cy = 0;
    function [15:0] goldp(input [31:0] xx, input [31:0] yy);
        goldp = (xx * yy) % M;
    endfunction

    // mirror the DUT's exact 4-cycle latency
    reg [15:0] e1,e2,e3,e4; reg ev1,ev2,ev3,ev4;
    ...
    initial begin
        ...
        $display("=================================================");
        $display(" Ansh-N (5-modulus) RTL Simulation  [Z/41580]");
        $display("=================================================");
