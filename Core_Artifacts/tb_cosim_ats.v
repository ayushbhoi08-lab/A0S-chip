...
module tb;
    ...
    initial begin
        ...
        $display("=== Ansh-108 A-TS notarizer host<->RTL co-sim TB ===");

        // ---- read the host-emitted vectors ---- //
        n=0;
        fd = $fopen("cosim_ats_vectors.txt", "r");
        if (fd==0) begin $display("  [FATAL] cannot open cosim_ats_vectors.txt"); $finish; end
        ...
        ofd = $fopen("cosim_ats_out.txt", "w");
        if (ofd==0) begin $display("  [FATAL] cannot open cosim_ats_out.txt"); $finish; end
